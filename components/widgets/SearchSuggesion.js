...
function SearchSuggestion({ keyword, showSuggestion }) {
	...
	return searchSuggestions.books?.length && showSuggestion ? (
			...
			<div
				...
				{searchSuggestions.authors.map((author, i) => (
					<Link href={`/authors/${author.slug}`} key={i}>
						<div
							className={'flex text-sm gap-2 px-2 py-1 cursor-pointer hover:bg-[#101621] text-white'}
							role='menuitem'
							tabIndex='-1'
							id='menu-item-0'>
							<Image
							...
							<div className='my-auto h-full px-1'>
								<p className='font-medium text-sm tracking-wide truncate text-white '>{author.name} </p>
							</div>
						</div>
					</Link>
				))}
