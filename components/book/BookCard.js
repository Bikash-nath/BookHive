		...
		<div
			...
			<Link href={`/books/${slug}`}>
				<div className='relative flex flex-col w-full h-full rounded-md p-2 bg-gray-700 bg-opacity-60 hover:bg-opacity-100 hover:scale-105 transform duration-200'>
					...
					/>
					<div key='content' className='px-1 flex flex-col'>
						<p
							key='title'
							className='font-semibold text-sm lg:text-base xl:text-base mt-1 lg:mt-0 leading-tight line-clamp-2 text-white '>
							{title}
						</p>
						<p key='author' className='text-xs font-thin lg:text-sm truncate text-gray-200 lg:py-1'>
							{`By ${author}`}
						</p>
					</div>
				</div>
